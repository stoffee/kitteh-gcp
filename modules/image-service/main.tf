...
resource "google_compute_region_instance_group_manager" "image_service" {
  ...
  depends_on = ["google_compute_instance_template.image_service_public", "google_compute_instance_template.image_service_private"]
}
...
resource "google_compute_instance_template" "image_service_public" {
  project = "${var.gcp_project_id}"
...
resource "google_compute_instance_template" "image_service_private" {
...
resource "google_compute_firewall" "allow_inbound_http_api" {
  ...
  name    = "${var.cluster_name}-rule-external-api-access-is"
  network = "${var.network_name}"
...
data "template_file" "compute_instance_template_self_link" {
  ...
  template = "${element(concat(google_compute_instance_template.image_service_public.*.self_link, google_compute_instance_template.image_service_private.*.self_link), 0)}"
}
