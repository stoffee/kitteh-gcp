...
output "instance_group_url" {
  value = "${google_compute_region_instance_group_manager.image_service.self_link}"
}

output "instance_group_name" {
  value = "${google_compute_region_instance_group_manager.image_service.name}"
}
...
output "instance_template_name" {
  value = "${element(concat(google_compute_instance_template.image_service_public.*.name, google_compute_instance_template.image_service_private.*.name), 0)}"
}

output "instance_template_metadata_fingerprint" {
  value = "${element(concat(google_compute_instance_template.image_service_public.*.metadata_fingerprint, google_compute_instance_template.image_service_private.*.metadata_fingerprint), 0)}"
}
